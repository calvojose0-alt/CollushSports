...
import { Users, Zap, Clock, CheckCircle2, AlertCircle, Search, ChevronDown, ChevronUp } from 'lucide-react'
import TeamLogo from '@/components/FootballWinLeague/TeamLogo'
import { NFL_WL_TEAMS } from '@/data/nflWinLeagueTeams'

// Tier names for the draft board grouping (32 teams → 4 tiers of 8)
const TIER_LABELS = {
  1: 'Tier 1 — Contenders',
  2: 'Tier 2 — Playoff Hopefuls',
  3: 'Tier 3 — In the Mix',
  4: 'Tier 4 — Rebuilding',
}
...
function Avatar({ name, size = 'sm', color = 'bg-green-700' }) {
...
// Pick order track showing all picks
    ...
    <div className="card p-0 overflow-hidden">
      ...
      <div className="px-4 py-3 flex flex-wrap gap-2">
        {order.slice(0, showCount).map((uid, idx) => {
          ...
          const pickedTeam = teamPick ? NFL_WL_TEAMS[teamPick.teamId] : null

          return (
            <div
              key={idx}
              className={`flex flex-col items-center gap-0.5 rounded-xl px-2.5 py-1.5 border transition-all ${
                isCur  ? 'border-green-500 bg-green-900/40 ring-1 ring-green-400' :
            ...
            >
              ...
              {isPast && pickedTeam ? (
                <TeamLogo team={pickedTeam} size="xs" />
              ) : (
                <Avatar name={p?.displayName} size="sm" color={isCur ? 'bg-green-600' : 'bg-f1light'} />
              )}
              <span className={`text-[10px] font-semibold truncate max-w-[52px] ${isCur ? 'text-green-300' : 'text-gray-400'}`}>
...
function RosterCard({ player, picks, isMe }) {
  return (
    <div className={`rounded-xl border p-3 ${isMe ? 'border-green-600 bg-green-900/20' : 'border-f1light bg-f1dark'}`}>
      <div className="flex items-center gap-2 mb-2">
        <Avatar name={player.displayName} color={isMe ? 'bg-green-700' : 'bg-f1light'} />
        <div className="min-w-0">
          <p className="text-xs font-bold text-white truncate">{player.displayName}</p>
          {isMe && <p className="text-[10px] text-green-400">You</p>}
        </div>
      </div>
        ...
        {picks.map((pick) => {
          ...
          return (
            <div key={pick.teamId} className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-gray-300 font-medium truncate">{team.name}</span>
            </div>
          )
        })}
...
export default function DraftPage() {
      ...
      <div className="space-y-4">
        <div className="card text-center py-14 space-y-3">
          ...
          <p className="text-gray-400 text-sm max-w-sm mx-auto">
            The admin is setting up the Pro Football Win-League. Check back soon — you'll be able to join once registration opens.
          </p>
        </div>
      </div>
    )
  ...
  if (status === 'open') {
    return (
      <div className="space-y-5">
        {/* Join card */}
        <div className="card border-green-700/40 bg-gradient-to-br from-green-900/30 to-green-800/10">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-xl bg-green-600 flex items-center justify-center">
              <Zap className="w-5 h-5 text-white" />
            </div>
            <div>
              <h2 className="font-bold text-white text-lg">Pro Football Win-League</h2>
              <p className="text-green-400 text-xs font-semibold">Registration Open</p>
            </div>
          </div>
          <p className="text-gray-300 text-sm mb-4 leading-relaxed">
            Draft 3 NFL teams. Earn 1 point for every win and 0.5 for every tie your teams pick up across the regular season. The player with the most points when Week 18 wraps is the Win League Champion!
          </p>
          <div className="flex items-center gap-3 mb-4">
          ...
          {!myPlayer ? (
            <button
              ...
              className="btn-primary w-full py-2.5 bg-green-600 hover:bg-green-500 disabled:opacity-50 flex items-center justify-center gap-2"
            >
              <Users className="w-4 h-4" />
              {joining ? 'Joining…' : 'Join Win League'}
            </button>
          ) : (
            <div className="flex items-center gap-2 text-sm text-green-400 bg-green-900/20 border border-green-700 rounded-lg px-3 py-2.5 font-semibold">
            ...
            <div className="divide-y divide-f1light">
              {players.map((p) => (
                <div key={p.id} className="px-4 py-2.5 flex items-center gap-3">
                  ...
                  {p.userId === user?.uid && (
                    <span className="ml-auto text-xs text-green-400 font-semibold">You</span>
                  )}
                </div>
              ))}
  ...
  if (status === 'drafting') {
    return (
      <div className="space-y-5">
        ...
        {/* On the clock banner */}
        {isMyTurn && (
          <div className="bg-green-900/50 border-2 border-green-500 rounded-2xl px-5 py-4 flex items-center gap-3 animate-pulse">
            <Zap className="w-6 h-6 text-green-400 flex-shrink-0" />
            <div>
              <p className="text-green-300 font-black text-base">It's your pick!</p>
              <p className="text-green-400 text-xs">Pick {currentPickIndex + 1} of {totalPicks} — choose a team below</p>
            </div>
          </div>
        )}
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full bg-f1dark border border-f1light rounded-lg pl-8 pr-3 py-1.5 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-green-500"
                />
              </div>
            </div>
                      ...
                      <button
                        ...
                        className={`relative flex items-center gap-2.5 rounded-xl border p-2.5 text-left transition-all ${
                          isDrafted
                            ...
                            : canPick
                            ? 'border-green-600/60 bg-green-900/20 hover:border-green-400 hover:bg-green-900/40 cursor-pointer'
                            : 'border-f1light bg-f1dark'
                        } ${draftedByMe ? 'ring-1 ring-green-500 opacity-70' : ''}`}
                      >
                        {/* Rank badge */}
                        <span className="absolute top-1 right-1.5 text-[9px] text-gray-600">#{team.rank}</span>

                        {isPickingThis ? (
                          <span className="inline-block w-7 h-7 border-2 border-green-400 border-t-transparent rounded-full animate-spin flex-shrink-0" />
                        ) : (
                        ...
                        {draftedByMe && (
                          <CheckCircle2 className="absolute top-1 left-1 w-3 h-3 text-green-400" />
                        )}
                      </button>
                    )
                  })}
  ...
  return (
    <div className="space-y-5">
      <div className="card border-green-700/30 bg-gradient-to-br from-green-900/20 to-transparent">
        <div className="flex items-center gap-3 mb-4">
          <CheckCircle2 className="w-6 h-6 text-green-400" />
          <div>
            <h2 className="font-bold text-white">Draft Complete</h2>
            <p className="text-xs text-green-400">All teams have been drafted — weekly scoring is live</p>
          </div>
        </div>
            ...
            .map((pick) => {
              ...
              return (
                <div
                  ...
                  className={`px-4 py-2.5 flex items-center gap-3 ${isMine ? 'bg-green-900/15' : ''}`}
                >
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {isMine && <span className="text-[10px] text-green-400 font-semibold">You</span>}
                  </div>
                </div>
              )
            })}
          ...
          {players.map((p) => {
            ...
            return (
              <div key={p.id} className="px-4 py-3 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                    {pPicks.map((pick) => {
                      const team = NFL_WL_TEAMS[pick.teamId]
                      return team ? (
                        <span key={pick.teamId} className="inline-flex items-center gap-1 text-xs text-gray-300">
                ...
                {p.userId === user?.uid && (
                  <span className="text-xs text-green-400 font-semibold flex-shrink-0">You</span>
                )}
              </div>
            )
          })}
